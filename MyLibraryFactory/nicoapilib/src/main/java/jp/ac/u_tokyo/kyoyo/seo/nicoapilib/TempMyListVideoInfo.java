...
/**
 * ニコ動ＡＰＩからのとりあえずマイリスト情報をパースします<br>
 * This parses temp myList response from Nico.<br><br>
 *
 * This class extending VideoInfoManager provides methods to parse JSON<br>
 * from http://www.nicovideo.jp/api/deflist/list<br>
 * note; getting temp my list needs login session stored in cookie<br><br>
 *
...
public class TempMyListVideoInfo extends VideoInfoManager {
    ...
    private TempMyListVideoInfo( JSONObject item)  throws NicoAPIException.ParseException {
        initialize(item);
    }
    ...
    /**
     * ニコ動APIからのとりあえずマイリスト情報をパースします<br>
     ...
     * ニコ動APIからのとりあえずマイリスト情報をパースします<br>
     * Parses temp myList response from Nico.<br>
     * APIやレスポンスの形式など詳細は{@link TempMyListVideoInfo ここから参照}してください。<br>
     ...
     * More details about API and response format are {@link TempMyListVideoInfo available here}.<br>
    ...
    protected static List<VideoInfo> parse(JSONObject root)  throws NicoAPIException {
        ...
        try{
            if (!root.getString("status").equals("ok")) {
                throw new NicoAPIException.APIUnexpectedException("Unexpected API response status > temp myList");
            }
